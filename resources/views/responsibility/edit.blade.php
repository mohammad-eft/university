...
<body>
    ...
    <form action="{{ route('resp.update') }}" method="post" class="w-1/2 m-auto border rounded-lg p-5">
        @csrf
        <input type="hidden" name="id" value="{{ $resp->id }}">
        <div class="mb-10">
            <label for="responsibility" class="text-lg font-normal text-gray-700 mb-3 inline-block">نام سمت :</label>
            <input type="text" name="responsibility" id="responsibility" class="w-full outline-none border rounded-md px-10 py-3" value="{{ $resp->responsibility }}">
        </div>
