...
<head>
    ...
    <title>ثبت دانشکده</title>
</head>
<body>
    @include('header')
    <h1 class="text-center text-3xl font-bold text-gray-700 my-10">ثبت دانشکده</h1>
    ...
    <form action="{{ route('college.store') }}" method="post" class="w-1/2 m-auto border rounded-lg p-5">
        @csrf
        <div class="mb-10">
            <label for="name" class="text-lg font-normal text-gray-700 mb-3 inline-block">نام دانشکده :</label>
            <input type="text" name="name" id="name" class="w-full outline-none border rounded-md px-10 py-3" placeholder="نام دانشکده را وارد کنید">
        </div>
        <h2 class="text-2xl font-bold text-gray-700">لیست دانشگاه ها :</h2>
        <select name="uni_id" class="my-5 mr-10">
            @foreach($unis as $uni)
            <option value="{{ $uni->id }}" class="text-lg font-normal text-gray-700 mb-3 inline-block"> دانشگاه {{ $uni->name . " " . $uni->city }}</option>
            @endforeach
        </select>
