...
<body>
    ...
    <form action="{{ route('lesson.update') }}" method="post" class="w-1/2 m-auto border rounded-lg p-5">
        @csrf
        <input type="hidden" name="id" value="{{ $lesson->id }}">
        <div class="mb-10">
            <label for="name" class="text-lg font-normal text-gray-700 mb-3 inline-block">نام درس :</label>
            <input type="text" name="name" id="name" class="w-full outline-none border rounded-md px-10 py-3" value="{{ $lesson->name }}">
        </div>
        <div class="mb-10">
            <label for="unit" class="text-lg font-normal text-gray-700 mb-3 inline-block"> تعداد واحد :</label>
            <input type="text" name="unit" id="unit" class="w-full outline-none border rounded-md px-10 py-3" value="{{ $lesson->unit }}">
        </div>
